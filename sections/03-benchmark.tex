...
The SAVNav task is situated in continuous indoor environments populated by static obstacles and dynamic human actors. The environment generates two classes of auditory signals: target acoustic events emitted by specific objects (e.g., doorbells, televisions) or humans (e.g., a human verbally calling the agent), and social acoustic events (e.g., footsteps, conversations) emitted continuously or intermittently by human actors. 

The objective of the embodied agent is to navigate to the designated acoustic target using egocentric visual and auditory sensors, alongside real-time pose estimation and a pre-computed map of the static environment. Crucially, the agent must treat the social acoustic events as boundaries. The agent does not have access to the ground-truth locations or trajectories of the human actors and must infer their presence—even when fully occluded by walls or corners—proactively yielding to avoid collisions and personal space violations.
...
To populate the simulated environment with realistic auditory cues, we curate a collection of 192 audio samples evenly distributed across the six defined sound event categories. Following the AudioSet Ontology~\cite{AudioSetOntology2017}, we organize these into target acoustic events (\texttt{doorbell}, \texttt{human call}, \texttt{television}, \texttt{water tap}) and social acoustic events (\texttt{conversation}, \texttt{footsteps}). These samples are sourced from BeDAViN~\cite{AudiovisualNavigationNoisy2025}, FSD50K~\cite{FSD50KOpenDataset2022}, and Freesound\footnote{\url{https://freesound.org/}} under Creative Commons licenses.
