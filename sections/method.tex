...
window (5s duration, 0.5s stride), ensuring a system latency of \(\sim\)0.5s despite the long temporal context. We utilize Embed-ACCDOA
...
\begin{enumerate}
    ...
    \item
          Hallucinated Moving Sources: For unanchored sources with dynamic signatures (e.g., footsteps), we generate a Lance-shaped Risk Field aligned with \(\mathbf{v}_{virt}\) to implement "Hearing before Seeing":
          \begin{equation}
          ...
          \end{equation}
          where \(d_{long}, d_{lat}\) are longitudinal and lateral distances. This encourages wide turns around corners.
    \item
...
\subsubsection{State B: Belief-Driven Active Search}\label{state-b-active-search}
...
\end{equation}
where \(\gamma(d)\) is a resolution decay factor that penalizes distant observations to ensure reliable detection. \(\mathcal{C}_{travel}\) represents the geodesic cost to the candidate pose. Upon visual confirmation, the specific entity is anchored, triggering a transition to State A.
