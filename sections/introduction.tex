...
\begin{figure}[t]
  ...
  \caption{\textbf{SAVNav Overview.} (Left) \textit{Task scenario:} Given an instruction (e.g., \emph{"Find the person calling for help"}), the robot navigates to an audio goal while avoiding humans. Crucially, it anticipates Non-Line-of-Sight (NLOS) risks by using the acoustic Direction-of-Arrival (DoA; orange cone) of unseen moving agents (e.g., footsteps) to proactively adjust its trajectory (blue). (Right) \textit{Execution flow:} Instructions, vision, and uncertainty-aware acoustics are fused into a spatio-temporal map and social costmap for risk-aware planning.}
  \label{fig:teaser}
\end{figure}

As mobile robots increasingly transition from controlled industrial environments to shared human spaces, the objective of navigation shifts from pure metric efficiency to social compliance. Social navigation requires robots to respect human comfort, maintain appropriate distances, and adhere to social norms \cite{PrinciplesGuidelinesEvaluating2025}. While significant progress has been made in modeling human-robot interaction, current systems often fail in highly dynamic, unstructured environments where safety is paramount.
...
This gap exists primarily due to a dichotomy in current research. Audio-Visual Navigation (AV-Nav) typically focuses on locating static sound sources (e.g., a phone is ringing) \cite{LookListenAct2020}, ignoring the social implications of sound as a dynamic movement cue. Conversely, Social Navigation focuses on avoidance but neglects the acoustic modality. Bridging this gap has been hindered by the lack of unified simulation infrastructure; high-fidelity acoustic simulators (e.g., SoundSpaces \cite{SoundSpaces20Simulation2022}) often lack the dynamic human agents found in social navigation platforms (e.g., Habitat-Lab \cite{Habitat30Cohabitat2023}). Consequently, there is no standardized testbed for developing agents that can navigate using the acoustic footprint of moving humans.
