...
To populate the simulated environment with realistic auditory cues, we curate a collection of 176 audio samples evenly distributed across the six defined sound event categories. Following the AudioSet Ontology~\cite{AudioSetOntology2017}, we organize these into target sound events (\texttt{doorbell}, \texttt{human call}, \texttt{television}, \texttt{water tap}) and social sound events (\texttt{conversation}, \texttt{footsteps}). These samples are sourced from BeDAViN~\cite{AudiovisualNavigationNoisy2025}, FSD50K~\cite{FSD50KOpenDataset2022}, and Freesound\footnote{\url{https://freesound.org/}} under Creative Commons licenses.
