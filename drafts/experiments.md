...
我们构建了一个统一仿真平台，集成了 **SoundSpaces 2.0** 的物理声学渲染与 **Habitat-Lab 3.0** 的动态人类智能体。
为了公平评估并防止数据泄露，我们采用了严格的数据划分策略：
...
1. 目标声音（Target Sounds）- 5 类：
   
   选择 MP3D 或 SoundSpaces 中现有的、且具有“定位需求”的声音：

    - Music / TV (固定源)：寻找正在播放音乐的音箱/电视（模拟“去关掉音乐”或“去客厅”）。
    - Water Running / Toilet Flush (固定源)：寻找正在流水的水槽/马桶（模拟“检查漏水”或清洁任务）。
